...
func (s *SbStatsService) RoutedConnectionInternal(inbound string, outbound string, user string, conn net.Conn, directIn bool) net.Conn {
	...
	s.access.Lock()
	if countInbound {
		readCounter = append(readCounter, s.getOrNewCounter("inbound>>>"+inbound+">>>traffic>>>uplink"))
		writeCounter = append(writeCounter, s.getOrNewCounter("inbound>>>"+inbound+">>>traffic>>>downlink"))
	}
	if countOutbound {
		readCounter = append(readCounter, s.getOrNewCounter("outbound>>>"+outbound+">>>traffic>>>uplink"))
		writeCounter = append(writeCounter, s.getOrNewCounter("outbound>>>"+outbound+">>>traffic>>>downlink"))
	}
	if countUser {
		readCounter = append(readCounter, s.getOrNewCounter("user>>>"+user+">>>traffic>>>uplink"))
		writeCounter = append(writeCounter, s.getOrNewCounter("user>>>"+user+">>>traffic>>>downlink"))
	}
	s.access.Unlock()
	if directIn {
...
// getOrNewCounter must be called with s.access held.
func (s *SbStatsService) getOrNewCounter(name string) *atomic.Int64 {
	counter, loaded := s.counters[name]
	if loaded {
		return counter
	}
